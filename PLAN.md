...
8. **⚠️ First thing next session:** no pressing gap flagged by this pass — candidates for whoever picks this up next: generalize the Rite-effect placeholder (`RiteEffectIds`) into the real card-effect system `design-continuous-effects.md` still calls undesigned; extend casting to Structure/Item once Track A fleshes those types out; or start on the actual Aether/trace model (D16) now that Hand/Library prove the zones pattern works. None of these block M1.5 playtesting, which is now fully usable end to end (move, attack, bond, draw, cast).
