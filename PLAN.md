...
| **M1.5** | Minimal playable prototype (debug UI) | **Tech pivot (2026-08-07):** a local web UI (no engine install needed, fast to iterate, scriptable for automated verification) instead of Godot for this throwaway debug stage — Godot is still the real M2 client. Thin local API server over `IHost`, plain HTML/JS board (hex terrain, text-labeled creatures, legal-actions-as-buttons, pending-stack respond/pass, P1/P2/true-state views), **plus a JSON scenario-file loader** so test positions don't require hand-written C# fixtures | A full match is actually playable, perceived-vs-true state can be eyeballed for consistency, and a new test scenario can be set up by editing a JSON file, not writing code |
...
6. **⚠️ Reprioritized (2026-08-07) — top of the list:** **Build M1.5** (not started), moved ahead of the Rite/Spell pipeline because real playtesting needs it now. Two parts, per the tech pivot in §7's M1.5 row: (a) a minimal local web UI over `IHost`'s `LegalCommands`/`Submit`/`CurrentView` surface, and (b) a JSON scenario-file loader so test positions are authored as data, not hand-written `MatchFactory` calls in C#. The Rite/Spell casting pipeline (`design-continuous-effects.md`'s next flagged gap — still no way to actually *cast* anything) comes after.
