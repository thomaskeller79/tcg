...
Supersedes an earlier same-session proposal ("client-local mirror + claim-based sync, validated against plausibility bounds"), which was correctly flagged as reopening the anti-cheat hole `docs/rules/history/decisions.md` D1 ("progression is server-authoritative") exists to close. Replaced by a mechanism that costs almost nothing extra, because the project already committed to determinism (pillar 4) and to recording/replaying command streams for balance analysis (`PLAN.md` §6):
