...
- **Declaring defenders (D4):** when a hex is attacked, the defending player **declares which of their creatures on that hex defend**. Default: **each creature may defend only once per turn** — the **pessimistic default (D14)**: "once" is the weak case, so *multi-defend* is a **positive keyword** (Bulwark/Guardian-type), not a negative "defends once" line. **Defend-cost rule — RESOLVED to `0*AP` (D15, 2026-08-09):** free, but at most once per turn per actor, completely decoupled from remaining AP. Rejected `1!AP` (exhaust): since default Attack already exhausts, the *first* combat between two creatures left both at 0 AP, but AP only refreshes on its own controller's turn — the attacker stayed defenseless through the opponent's entire following turn while the defender refreshed almost immediately, making attacking first a liability. Rejected flat free-and-unlimited too: it fixes the same bug but drops the per-turn cap. `0*AP` keeps the cap, loses the bug. `cannot defend` is an occasional negative keyword; a card can still deliberately consume a creature's Defend as part of a strong ability's own effect (opt-in, card-level — see D15).
...
- **A Companion (D22) is a second, independent root:** it bonds terrain the same way (its own `x*AP: Bond`, reachable from itself), but the mana feeds a **private pool** only that Companion spends — never the shared pool. Ownership follows whichever root performed the bond. See `design-companions.md`.
...
- **Default:** each player has exactly one **Champion** on the board, placed at start on its **home tile** — and, **under test since 2026-08-08**, already bonded to that tile (1 mana live from turn 1, rather than needing a live Bond + a full turn cycle first). It is a special card type (HP, position) simulated by the core; **it is attackable and its death loses the game** (§7, D9).
- **Mana + Action Points (D9, revised 2026-08-06):** the Champion runs the **same two-resource shape as a creature** — *mana* (spent on spells/units, many/turn) and its own **AP** (private, refills each turn), spent on **draw a card, bond a terrain, move, fight, activate abilities** — not a bespoke third model. Baseline example (tuning): 7 AP; `5*AP` draw (replaces the automatic per-turn draw), `2*AP` bond (the D8 once/turn limit), `2AP`/`1AP` move (see the realm constraint below), `5!AP`/`3!AP` attack (network-active/disconnected — drain-all like a Creature's default attack). No hardcoded "pick one" — see `design-champions.md` for the full breakdown, the tension, and open items (ability list, defend/retaliation).
- **Mana is a per-turn snapshot, under test (2026-08-08):** mana is refreshed once per Beginning phase (the sum of currently-connected/producing bonded terrain at that instant) — a bond made mid-turn is permanent but doesn't credit the pool until the *following* Beginning phase, the same "no retroactive unlock this turn" shape as summoning sickness (D14).
- **Realm-bound movement (D9, D8) — tightened, under test (2026-08-08):** the Champion is the network root. While its network is currently connected, it is **confined to its own bonded territory** — it may reposition onto any hex it has itself bonded, at `2AP`, but may never step onto ground it hasn't claimed. To go anywhere else it must first pay `0AP` for the **Collapse Network** ability, which drops every bond outright (not a reversible pause — they're gone, re-bond from scratch); once disconnected it moves freely at `1AP` like any creature. *(An earlier, looser draft of this rule let the Champion drift one hex past its actual bonded tiles while "staying connected" — closed the same day once playtesting caught it.)*
