...
**Decisions:** D3–D6, D8–D14, D16, D18, D20, D21, D24, D26, D30 (`history/decisions.md`)
...
- **A hex also has an independent Structure slot (D24), capacity 1** — not a layer, doesn't consume Ground/Above/Below capacity. Controlled by whoever currently bonds that terrain cell (mirrors terrain control, §5), not by whoever built it. See `structures-items.md`.
...
- **Default:** each player has exactly one **Champion** on the board, placed at start on its **home tile**, already bonded to that tile (1 mana live from turn 1, rather than needing a live Bond + a full turn cycle first). It is a special card type (HP, position) simulated by the core; **it is attackable and its death loses the game** (§7, D9).
- **Mana + Action Points (D9):** the Champion runs the **same two-resource shape as a creature** — *mana* (spent on spells/units, many/turn) and its own **AP** (private, refills each turn), spent on **draw a card, bond a terrain, move, fight, activate abilities** — not a bespoke third model. Baseline example (tuning): 7 AP; `5*AP` draw (replaces the automatic per-turn draw), `2*AP` bond (the D8 once/turn limit), `2AP`/`1AP` move (see the realm constraint below), `5!AP`/`3!AP` attack (network-active/disconnected — drain-all like a Creature's default attack). No hardcoded "pick one" — see `champions.md` for the full breakdown, the tension, and open items (ability list, defend/retaliation).
