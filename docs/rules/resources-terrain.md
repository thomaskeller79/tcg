...
- **Terrain's own Activation Points (D58):** Terrain holds its own Activation Points like any other non-Item permanent — an explicitly printed value, not a silent default — self-funding the AP portion of its own printed ability directly (no climbing). Terrain gets full Activation Points immediately on entering play, since it's placed at match setup rather than cast (D57) — not a type-based exception, casting simply never applies to it.
