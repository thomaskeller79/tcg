...
| **Structure** | Stationary, **non-carryable** Play-permanent (D17); subtypes e.g. *Building*. Absorbs non-equippable "artifacts." *(Open: whose AP/mana funds its own abilities — working hypothesis is the Champion's, see `design-economy.md`.)* |
| **Item** | **Carryable** Play-permanent (D17): can be picked up / equipped. *(Open: whose AP/mana funds its own abilities — working hypothesis is the carrying creature's AP + shared mana, see `design-economy.md`.)* |
...
| **Connection / Bond** | The AP-costed act (`x*AP`, at most once/turn) of adding one terrain to a network **rooted at the Champion or a Companion** (D22), via already-bonded terrain reachable from that root. Bonding is **permanent**; the ramp mechanism. |
| **Bonded** | A terrain added to a player's network by whichever root performed the bond. Produces mana only while an enemy-free path connects it back to **its owning root**; feeds the shared pool if Champion-bonded (permanent, D8), or that Companion's private pool if Companion-bonded (D22) — in which case it reverts to unbonded if that Companion dies, rather than staying permanent. |
