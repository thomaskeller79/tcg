...
**Narrowed by D26 (ownership tree, `ownership.md`):** "private" doesn't mean "usable only by the Companion card itself" — a Companion funds **everything in its own subtree** (creatures it produces via its own abilities, terrain/structures it bonds, items equipped to those creatures) from this same private pool. What it still can't do is fund a *different* payer's own abilities or spells directly (the Champion's, or a different Companion's) — the boundary above is about crossing between pools, not about the Companion being an island.
