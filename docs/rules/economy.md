...
**Decisions:** D8 (terrain/mana), D9 (Champion economy), D10 (Action Points), D15 (Defend cost), D22 (Companion), D23 (instant-speed marker) — `history/decisions.md`
...
### The `^` marker (instant-speed, D23)
A third axis, independent of `!`/`*`: **`^`** prefixed on any AP cost marks that specific *ability* as playable **instant-speed** — usable in any priority window (D5), on either player's turn. **Default (no `^`): sorcery-speed** — usable only during the actor's controller's own Action phase, and only while the stack (the Aether's pending region, D16) is empty. Generalized to the **ability** level rather than the card level: D5's existing instant/sorcery split already covers whole cards; `^` lets individual abilities *on the same card* differ (a Champion might have one sorcery-speed ability and one reactive one).

Composes freely with `!`/`*`: `^2*AP`, `^3!AP`, `^1AP` are all valid — `^` says *when*, `!`/`*` say *how the AP is consumed*. Spending AP reactively still draws from the actor's normal AP budget, never a separate reactive pool — so using an instant-speed ability on the opponent's turn means that AP had to be held in reserve since the actor's own last turn. Applies generally, for any actor (creature, Champion, Companion), not as a Champion-specific rule.

A fourth flavor, `x!!AP` ("double-exhaust"), is a proposed-not-adopted alternative — see `history/playtest-variants.md`.
...
Total systems: **mana + AP + terrain network + stack + perception**. Mitigation: every AP cost is a **small integer**, resist per-action special cases, let depth come from combinations (pillar 3, tension #6 in PLAN).
