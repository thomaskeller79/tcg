...
{
    [Fact]
    public void Damage_spell_hits_the_target_spends_mana_and_leaves_no_permanent()
    {
    ...
    [Fact]
    public void Heal_spell_restores_life()
    {
    ...
    [Fact]
    public void A_lethal_damage_spell_wins_the_match_via_the_existing_win_check_no_special_casing()
    {
        var match = SpellFixtures.ChampionWithMana(mana: 5, hand: [SpellFixtures.Firebolt]);
        var enemyChampion = match.State.ActorsOwnedBy(Fixtures.P2).Single();
    ...
    public void Cannot_target_a_concealed_enemy_you_cannot_see()
    {
        ...
        var result = RulesEngine.Apply(match, new CastSpellCommand(Fixtures.P1, SpellFixtures.Firebolt, hiddenEnemy));
